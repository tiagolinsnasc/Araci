...
func _on_activation_area_area_entered(area: Area2D) -> void:
	if area.name == "hurtbox":
		...
		anime.play("action")
		#Para só ativar uma vez a dinamite
...
func _on_anime_animation_finished() -> void:
	if anime.animation == "explosion":
		...
		# espera a duração do som antes de eliminar
...
func _on_damage_area_area_entered(area: Area2D) -> void:
	if area.name == "hurtbox":
		...
		# pega o nó pai do hurtbox (que é o player)
		var player = area.get_parent()
			
		# aplica knockback e chama a função de dano
